...
struct DetailView: View {
    ...
    var body: some View {
        ZStack {
            AsyncImage(url: URL(string: "\(imagesUrlPrefix)\(movie?.backdrop_path ?? "")")) { phase in
            ...
            VStack(alignment: .leading) {
                ...
                Text(movie?.overview ?? "").foregroundColor(Color.white).bold().font(.system(size: 16))
                Text("aaaaaaaaaa").foregroundColor(Color.white).bold().font(.system(size: 16))
            }.padding()
        }.clipped()
    }
}
